...
class TestRecycleBin(unittest.TestCase):
    ...
    def test_recyclebin_get_disabled(self):
        ...
        with mock.patch("plone.restapi.services.recyclebin.get.getUtility", 
    ...
    def test_recyclebin_get_enabled_with_items(self):
        ...
        self.assertEqual(
            self.portal.absolute_url() + "/@recyclebin-purge", 
            item1["actions"]["purge"]
        )
    ...
    def test_restore_disabled_recyclebin(self):
        ...
        with mock.patch("plone.restapi.services.recyclebin.restore.getUtility", 
                         return_value=recycle_bin):
            response = self.api_session.post(
                "/@recyclebin-restore", 
                json={"item_id": "123456789"}
            )
    ...
    def test_purge_disabled_recyclebin(self):
        ...
        with mock.patch("plone.restapi.services.recyclebin.purge.getUtility", 
                         return_value=recycle_bin):
            response = self.api_session.post(
                "/@recyclebin-purge", 
                json={"item_id": "123456789"}
