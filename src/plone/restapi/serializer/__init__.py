...
def lookup_at_field_serializer(context, field):
    objects = (field, context, context.REQUEST)
...
def SerializeATToJson(context):
    ...
    if IATCollection.providedBy(context):
        portal = getSite()
        result['member'] = [
            {
                '@id': '{0}/{1}'.format(
    ...
    pairs = [(field.getName(), field) for field in context.Schema().fields()]
    for field_name, field in pairs:
        value = lookup_at_field_serializer(context, field)()
        result[json_compatible(field_name)] = value
